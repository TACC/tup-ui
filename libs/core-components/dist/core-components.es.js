...
const Icon = ({
  ...
  const fullClassName = className ? [className, iconClassName].join(" ") : iconClassName;
...
const Button = ({
  ...
  if (type === "primary" && size === "small") {
    type = "secondary";
    console.error('A <Button type="primary" size="small"> is not allowed. Using `type="secondary"` instead.');
  }
  if (type !== "link" && !size) {
    size = "short";
    console.debug('A <Button> that is not `type="link"` and has no `size` is automatically assigned `size="short"`.');
  }
  return /* @__PURE__ */ jsxs("button", {
    ...
    children: [isLoading && /* @__PURE__ */ jsx(LoadingSpinner, {
    ...
    }), iconNameBefore ? /* @__PURE__ */ jsx(Icon, {
      name: iconNameBefore,
      ...
      "data-testid": "text",
      children
    }), iconNameAfter && /* @__PURE__ */ jsx(Icon, {
      name: iconNameAfter,
